...
image = (
    ...
    .add_local_file("modal_app_roxford_eval.py", "/root/modal_app_roxford_eval.py")
)

# Define Modal App with dataset volume
app = modal.App(
    "ROxford Evaluation with Official Protocol",
    image=image,
...
def _load_image(path):
    ...
    preprocess = transforms.Compose([
        ...
        transforms.CenterCrop(model_img_size),  # Training augmentation - match img_size
        transforms.ToTensor(),
...
def main(
    ...
    steps: int = 1,
    batch_size: int = 64,   # Back to original size - DataLoader workers now handle I/O efficiently
    report_interval: int = 1,
    eval_batch_size: int = 64,  # Base eval batch size (will be multiplied by num_gpus)
    ...
    def compute_ap(ranks, nres):
        ...
        ranks : 1D array of zero‐based ranked indices of relevant images
        nres  : number of relevant images
        """
        if nres == 0:
            return 0.0
        # sort ranks ascending, convert to 1-based positions
        ranks = np.sort(ranks) + 1
        # precision at each retrieved relevant
        precision = np.arange(1, ranks.size + 1) / ranks
        return precision.sum() / nres

    def evaluate_roxford(model, device,
                         roxford_root: str = "/mnt/data/roxford5k",
                         ks=(1,2,4),
                         batch_size: int = 100):
        """
        Official ROxford evaluation using ground truth pickle file.
        Works with both original and converted ROxford dataset structures.
        """
        print("🏛️ Starting official ROxford evaluation...")
        
        # 1) Load ground truth
        with open(os.path.join(roxford_root, "gnd_roxford5k.pkl"), "rb") as f:
            data = pickle.load(f)
        imlist   = data["imlist"]
        qimlist  = data["qimlist"]
        gnd      = data["gnd"]

        print(f"📊 Loaded {len(imlist)} gallery images, {len(qimlist)} queries")

        # 2) Build image path lookup for converted dataset structure
        def find_image_path(img_name, roxford_root):
            """Find image path in either original or converted structure."""
            # Try original structure first: jpg/image_name.jpg
            original_path = os.path.join(roxford_root, "jpg", img_name + ".jpg")
            if os.path.exists(original_path):
                return original_path
            
            # Try converted structure: search in class directories (0/, 1/, 2/, etc.)
            for class_dir in os.listdir(roxford_root):
                class_path = os.path.join(roxford_root, class_dir)
                if os.path.isdir(class_path) and class_dir.isdigit():
                    # Look for the image in this class directory
                    for ext in ['.jpg', '.jpeg', '.png']:
                        img_path = os.path.join(class_path, img_name + ext)
        ...
        # 3) Build gallery and query embeddings
        model.eval()
        
        # --- gallery ---
        gallery_embeddings = []
        valid_gallery_indices = []
        print("🖼️ Extracting gallery embeddings...")
        for i, img_name in enumerate(tqdm(imlist, desc="Gallery")):
            img_path = find_image_path(img_name, roxford_root)
            if img_path is not None:
                img = _load_image(img_path)
                with torch.no_grad():
                    emb = model(img.unsqueeze(0).to(device))[0,0].cpu().numpy()
                gallery_embeddings.append(emb)
                valid_gallery_indices.append(i)
            else:
                print(f"⚠️  Gallery image not found: {img_name}")
        
        if len(gallery_embeddings) == 0:
            print("❌ No gallery images found! Check dataset structure.")
            return 0.0
        ...
        for i, qname in enumerate(tqdm(qimlist, desc="Queries")):
            img_path = find_image_path(qname, roxford_root)
            if img_path is not None:
                img = _load_image(img_path)
                with torch.no_grad():
                    emb = model(img.unsqueeze(0).to(device))[0,0].cpu().numpy()
                ...
                print(f"⚠️  Query image not found: {qname}")
        
        if len(query_embeddings) == 0:
            print("❌ No query images found! Check dataset structure.")
            return 0.0
            
        queries = np.vstack(query_embeddings)
        queries = queries / np.linalg.norm(queries, axis=1, keepdims=True)
        print(f"✅ Loaded {len(query_embeddings)} query embeddings")
        
        # Check if we have enough data for evaluation
        if len(gallery_embeddings) < 10 or len(query_embeddings) < 5:
            print(f"⚠️  Insufficient data for reliable evaluation:")
            print(f"   Gallery: {len(gallery_embeddings)} images (need >10)")
            print(f"   Queries: {len(query_embeddings)} images (need >5)")
            print("   Returning default score.")
            return 0.0

        # 4) Precompute similarity matrix
        print("📈 Computing similarities...")
        sims = queries @ gallery.T  # shape [Q x G]
        ranks = np.argsort(-sims, axis=1)  # descending similarity

        # 5) For each difficulty, compute AP per query (only for valid queries)
        maps = {"Easy": [], "Medium": [], "Hard": []}
        
        # Create mapping from original indices to valid indices
        gallery_idx_map = {orig_idx: new_idx for new_idx, orig_idx in enumerate(valid_gallery_indices)}
        
        # Map original gallery indices to new indices (only keep valid ones)
        def map_indices(indices):
            mapped = []
            for idx in indices:
                if idx in gallery_idx_map:
                    mapped.append(gallery_idx_map[idx])
            return np.array(mapped, dtype=int)
        
        for new_qi, orig_qi in enumerate(valid_query_indices):
            entry = gnd[orig_qi]
            # zero-based indices of positives (convert to gallery indices)
            easy = np.array(entry["easy"], dtype=int) - 1
            hard = np.array(entry["hard"], dtype=int) - 1
            allpos = np.union1d(easy, hard)
            junk = np.array(entry["junk"], dtype=int) - 1

            easy_mapped = map_indices(easy)
            hard_mapped = map_indices(hard)
            allpos_mapped = map_indices(allpos)
            junk_mapped = map_indices(junk)

            # remove junk from ranking
            rank = ranks[new_qi]
            if len(junk_mapped) > 0:
                mask = np.in1d(rank, junk_mapped, invert=True)
                rank = rank[mask]

            for split, pos in [("Easy", easy_mapped), ("Medium", allpos_mapped), ("Hard", hard_mapped)]:
                # intersect only relevant for this split
                pos = pos[pos >= 0]
                if pos.size == 0:
                    maps[split].append(0.0)
                    continue
                # find positions of `pos` in `rank`
                hits = np.where(np.in1d(rank, pos))[0]
                ap = compute_ap(hits, pos.size)
                maps[split].append(ap)

        # 5) Report mAP@k (though mAP is independent of k, but you can still report recall@k)
        print("\n" + "="*50)
        print("🏆 OFFICIAL ROXFORD EVALUATION RESULTS")
        print("="*50)
        header = f"{'Split':>6}  mAP"
        print(header)
        print("-" * len(header))
        for split in ("Easy", "Medium", "Hard"):
            mAP = np.mean(maps[split]) * 100
            print(f"{split:>6}  {mAP:6.2f}")
        ...
        for k in ks:
            recalls = []
            for new_qi, orig_qi in enumerate(valid_query_indices):
                pos = np.union1d(np.array(gnd[orig_qi]["easy"]) - 1,
                                 np.array(gnd[orig_qi]["hard"]) - 1)
                # Map to valid gallery indices
                pos_mapped = map_indices(pos)
                rank_k = ranks[new_qi][:k]
                recalls.append((np.intersect1d(rank_k, pos_mapped).size > 0)*1)
            print(f"{'Recall@'+str(k):>12}  {100*np.mean(recalls):6.2f}%")
        print("="*50)

        model.train()
        return np.mean(maps["Medium"])  # Return medium mAP as main metric
    ...
    # Try to run official ROxford evaluation if ground truth file exists
    official_map = None
    roxford_paths_to_try = [
        "/mnt/data/roxford5k",           # Original dataset location
        "/mnt/data/roxford5k_converted", # Converted dataset location  
        str(roxford_root_path)           # User specified location
    ]
    
    for path in roxford_paths_to_try:
        pickle_path = os.path.join(path, "gnd_roxford5k.pkl")
        if os.path.exists(pickle_path):
            print(f"✅ Found ground truth file at: {pickle_path}")
            official_map = evaluate_roxford(
                model, device,
                roxford_root=path,
                ks=[1, 2, 4], 
            ...
            print(f"❌ Ground truth file not found at: {pickle_path}")
    
    if official_map is None:
        print("⚠️  Official ROxford evaluation skipped - ground truth file not found")
        print("   To run official evaluation, ensure gnd_roxford5k.pkl is available")
        official_map = 0.0  # Default value
