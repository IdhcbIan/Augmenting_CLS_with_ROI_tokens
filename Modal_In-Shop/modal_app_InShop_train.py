...
app = modal.App(
    "In-Shop Instance Retrieval Training",
    image=image,
...
def main(
    ...
    lr: float = 1e-5,
    items_per_batch: int = 8,  # Number of different items per batch
    samples_per_item: int = 8,   # Number of samples per item 
    hard_negative_ratio: float = 0.1,  # Use top 10% hardest negatives
    ...
    Args:
        ...
        temperature: Temperature for straight-through estimator
    """
    print(f"Starting In-Shop Instance Retrieval Training with Hard Negative Mining")
    ...
    # Setup optimizer and hard negative mining loss
    ...
    dataloader = DataLoader(
        dataset, 
        batch_size=1,  # Dataset returns complete batches
        shuffle=True,
    ...
    for i, batch in enumerate(tqdm(dataloader, desc="In-Shop Training (Hard Negative Mining)", total=steps)):
        if i >= steps:
            break
            
        # Unpack batch
        images, labels = batch[0].squeeze(0), batch[1].squeeze(0)
    ...
    checkpoint_dir = Path(f"/mnt/data/Checkpoints/inshop_hardneg_{timestamp}")
    ...
    checkpoint = {
        ...
        'training_config': {
            ...
            'training_method': 'hard_negative_mining_inshop_instance_retrieval',
    ...
    with open(config_path, 'w') as f:
        f.write(f"In-Shop Instance Retrieval Training - {timestamp}\n")
        f.write("=" * 80 + "\n")
        ...
        f.write(f"  - Hard negative ratio: {hard_negative_ratio}\n")
...
if __name__ == "__main__":
    # Example usage for In-Shop instance retrieval training
    with app.run():
        result = main.remote(
            steps=1000, 
            items_per_batch=8,
        ...
        print("In-Shop Training Result:", result) 
