...
app = modal.App(
    "CARS196 EVAL HARDNEGATIVE ESTIMATOR PADDING AND SCALING",
    image=image,
...
class TrainableMultiVectorEncoder(nn.Module):
    """TRAINABLE Multi-vector encoder with straight-through estimator"""
    ...
    def __init__(self):
        ...
        self.embed_dim = 384     # Our desired output dimension(For comparison in Big Table!!)
    ...
    def _buddy_pool(self, cue, patches2d, valid_masks=None):
        """Improved buddy pooling with straight-through estimator"""
...
class GalleryDataset(Dataset):
    ...
    def __init__(self, file_list, cars_root):
        self.file_list = file_list  # List of (class_label, file_path) tuples
        self.cars_root = Path(cars_root)
    ...
    def __getitem__(self, idx):
        class_label, file_path = self.file_list[idx]
        
        try:
            img = Image.open(file_path).convert("RGB")
            img_tensor = self.transform(img)
            return img_tensor, class_label, str(file_path)
        except (FileNotFoundError, OSError) as e:
            print(f"  Error loading {file_path}: {e}")
            # Return a dummy tensor
            dummy_tensor = torch.zeros(3, 224, 224)
            return dummy_tensor, class_label, str(file_path)
...
class EfficientMultiScaleQueryDataset(Dataset):
    ...
    def __init__(self, query_list, padding_scales=[1.0, 1.1, 1.2, 1.3], valid_threshold=0.8):
    ...
    def __getitem__(self, idx):
        class_label, query_path = self.query_list[idx]  
        ...
        # Stack all scales for this query.
        stacked_tensors = torch.stack(all_scales_tensors)  
        stacked_masks = torch.stack(all_scales_masks)      
        
        return stacked_tensors, stacked_masks, class_label, query_path
...
def parse_cars196_for_eval(root: Path):
    """Parse Cars196 dataset for evaluation: ALL train images for gallery, ALL test images for queries"""
    print("Starting Cars196 parsing for evaluation...")
    ...
    # Parse ALL training images for gallery
    print(" Parsing training images for gallery...")
    ...
    for annotation in train_annotations:
        try:
            if len(annotation) >= 6:
                fname = str(annotation[5][0])  
                class_id = int(annotation[4][0][0]) - 1  
                
                if class_id < len(class_names):
                    class_name = class_names[class_id]
                    image_path = str(root / "cars_train" / fname)
                    train_files.append((class_to_idx[class_name], image_path))
                    
        except (ValueError, IndexError) as e:
            print(f"  Error parsing train annotation {annotation}: {e}")
            continue
    
    # Parse ALL test images.
    print(" Parsing test images for queries...")
    test_annos = scipy.io.loadmat(root / "cars_test_annos_withlabels.mat")
    ...
    for annotation in test_annotations:
        try:
            if len(annotation) >= 6:
                ...
                if class_id < len(class_names):
                    class_name = class_names[class_id]
                    image_path = str(root / "cars_test" / fname)
                    test_files.append((class_to_idx[class_name], image_path))
                    
        except (ValueError, IndexError) as e:
            print(f"  Error parsing test annotation {annotation}: {e}")
            continue
    
    print(f"   Cars196 Evaluation Dataset Statistics:")
    print(f"   - Gallery (train): {len(train_files)} images")
    print(f"   - Queries (test): {len(test_files)} images")
...
def evaluate_retrieval_cars196_dataset(model, checkpoint_dir, cars_root, train_files, test_files, eval_batch_size, device, ks=[1, 2, 4, 8], valid_threshold=0.8, padding_scales=[1.0, 1.1, 1.2, 1.3]):
    print(f"\nCars196 evaluation")
    print("="*50)
    
    if not train_files:
        print(f" No train files found in Cars196 dataset")
        return None

    # Build gallery from train files using DataLoader
    print(f" Creating gallery dataset from {len(train_files)} train files...")
    gallery_dataset = GalleryDataset(train_files, str(cars_root))
    ...
    gallery_stack = torch.stack(gallery_embeddings).to(device)
    ...
    if not test_files:
        print(f" No test files found in Cars196 dataset")
        return None
    
    print(f"\n processing of ALL scales in parallel: {padding_scales}")
    ...
    print(f" Processed all queries: {len(all_query_embeddings)} queries × {num_scales} scales")
    ...
    print(f" Efficiency: ALL scales processed in parallel batches (5-10x speedup!)")
    for k in ks:
    ...
    with open(results_file, 'w') as f:
        ...
        f.write("="*60 + "\n")
        f.write(f"Model: CUB Aggressive Hard Negative Mining + Straight-Through Estimator\n")
...
@app.function(
    gpu="A100-80GB:1",  # 1 A100-80GB GPU for evaluation
    timeout=7500  # 2.5 hour timeout
)
def main(
    cars_root: str = "/mnt/data/stanford_cars",
    checkpoint_path: str = "/mnt/data/Checkpoints/cars196_hardneg_estimator_20250810_191717",
    eval_batch_size: int = 64,
    padding_scales = [0.1, 0.2, 0.4, 0.5, 0.6, 0.7, 0.8, 1.0, 1.2, 1.4],  # List of padding scale factors
    valid_threshold: float = 0.8,
    max_images_per_class: int = 50
):
    """
    Evaluate a trained Cars196 Hardnegative Estimator model on FULL dataset using MAX combination across multiple padding scales.
    
    Args:
        cars_root: Path to stanford_cars directory
        checkpoint_path: Path to saved checkpoint directory
        eval_batch_size: Batch size for evaluation (smaller due to multiple scales)
        padding_scales: List of padding scale factors to try
        valid_threshold: Threshold for valid patches (0.0-1.0)
    """
    if padding_scales is None:
        padding_scales = [1.0, 1.1, 1.2, 1.3]  # Multiple scales for robust evaluation
        
    print(f" Starting Cars196 Hardnegative Estimator Model Evaluation with MAX Combination")
    print(f"   Dataset: Stanford Cars196 (FULL DATASET)")
    print(f"   Gallery: ALL cars_train/ images")
    print(f"   Queries: ALL cars_test/ images")
    ...
    # Load Cars196 dataset for evaluation
    train_files, test_files = parse_cars196_for_eval(cars_root_path)
    
    if not train_files or not test_files:
        print(" No train or test files found in Cars196 dataset")
        return None
    
    print(f" Cars196 Dataset Loaded for Evaluation:")
    print(f"   Train files: {len(train_files)}")
    print(f"   Test files: {len(test_files)}")
    
    # Using ALL images as requested - no limiting
    print(f" Using FULL dataset:")
    print(f"   Gallery (train) files: {len(train_files)}")
    print(f"   Query (test) files: {len(test_files)}")
    ...
    # Evaluate on Cars196 dataset
    print(f"\n{'='*20} Stanford Cars196 with MULTIPLE PADDINGS {'='*20}")
    
    try:
        results = evaluate_retrieval_cars196_dataset(
            model, checkpoint_dir, cars_root_path, train_files, test_files, eval_batch_size, device,
...
if __name__ == "__main__":
    # Example usage
    with app.run():
        # Evaluate trained Cars196 model
        result = main.remote(
            checkpoint_path="/mnt/data/Checkpoints/cars196_aggressive_hardneg_estimator_20250101_120000",
            eval_batch_size=32,
            padding_scales=[1.0, 1.1, 1.2, 1.3],
            valid_threshold=0.8
        )
        print("Cars196 Hardnegative Estimator Evaluation Result:", result) 
